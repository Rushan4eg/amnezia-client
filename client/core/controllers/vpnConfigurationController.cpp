                                                                         ...
                                                                         const QString &stdOut)
{
    ...
    if (container == DockerContainer::Telemt) {
        ...
        if (mSecret.hasMatch()) {
            protocol.insert(protocols::telemt::secretKey, mSecret.captured(1));
        }
        if (mTgLink.hasMatch()) {
            protocol.insert(protocols::telemt::tgLinkKey, mTgLink.captured(1));
        }
        if (mTmeLink.hasMatch()) {
            protocol.insert(protocols::telemt::tmeLinkKey, mTmeLink.captured(1));
        }
